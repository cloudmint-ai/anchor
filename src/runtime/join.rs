...
tests! {
    ...
    async fn test_spawn() {
        ...
        for id in vec![1, 2, 3, 4, 5, 6] {
            let handle = spawn(async move { ok(id).await });
            task_handles.push(handle);
        }
        assert_eq!(join_all(task_handles).await?, vec![1, 2, 3, 4, 5, 6]);

        let mut task_handles = Vec::new();
        for id in vec![1, 2, 3, 4, 5, 6] {
            let handle = if id == 5 {
                spawn(async move { unexpected().await })
            } else {
                spawn(async move { ok(id).await })
            };
            task_handles.push(handle);
        }
        if let Ok(_) = join_all(task_handles).await {
            panic!("it should err")
        }

        let mut task_handles = Vec::new();
        for id in vec![1, 2, 3, 4, 5, 6] {
            let handle = spawn(async move { Ok(id) });
            task_handles.push(handle);
        }
        assert_eq!(join_all(task_handles).await?, vec![1, 2, 3, 4, 5, 6]);
    }
