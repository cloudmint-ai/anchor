...
// TODO 让 service 里的 config 不在是一个独立的子crate

// TODO 删除RwLock 而直接用实际的初始化
// TODO 另外是否能识别 engine 和 非engine 项目
...
#[cfg(test)]
mod tests {
    #[cfg(any(feature = "wasm", feature = "runtime"))]
    use super::*;
    ...
    async fn test_config_from_sample() {
        let toml_str = runtime::fs::read_to_string("src/_config/config.toml.sample").await?;
